...
- `uncertain_homography_solver` takes in the minimal set and returns an uncertain homography.

The points flip check is not being performed.


"""
function synthetic_labelling_test(noise_m₁, noise_m₂, T₁::Float64, T₂::Float64, T₃::Float64, outlier_ratio::Float64, n_corresps::Integer, n_runs::Integer, uncertain_homography_solver)

    @assert T₁ >= 0.0 "Can't have a negative threshold statistic"
    @assert T₂ >= 0.0 "Can't have a negative threshold statistic"
    @assert 0.0 <= outlier_ratio < 1.0 "An ivalid outlier ratio"

    image_bounds = [800.0, 1200.0]
    ...
    for _ in 1:n_runs
        ...
        uncertain_homography = uncertain_homography_solver(minimal_set)
        RDR.swap_points_in_correspondence!.(minimal_set)
        uncertain_homography_inv = uncertain_homography_solver(minimal_set)
    ...
    return confusion_matrix_labelling, [ForwardStatistics, TwoWayStatistics, ReprojectionStatistics, UniformStatistics, CovRANSACStatistics], TrueLabels
end

# ╔═╡ 26827270-dd23-46f5-a72e-3462d36f65ae
begin
    labels = ["Forward residual", "Two 1-way residuals", "Reprojection residual", "Uniform thresholding", "Ellipses intersection"]

    function run_labelling_experiments(uncertain_H_solver, total_runs, coeff)
        ...
        conf_m_labelling, raw_statistics, TrueLabels = synthetic_labelling_test(
            coeff * [1.2 -0.9; -0.9 2.5],
            coeff * [1.8 0.3; 0.3 1.7],
            point_identity_st,
		...
		return conf_m_labelling, raw_statistics, TrueLabels	
    end
...
# ╔═╡ 4273c30c-5b45-4d2d-b310-8c029a56647b
begin
	conf_m_labelling, _, _ = run_labelling_experiments(RDR.compute_uncertain_homography, 200, 1)
	...
	md"""
	#### Consensus Set Labelling Analysis | Sanity Check
...
begin
	for (coeff, noise_label) in zip([1., 10.], ["low_noise", "large_noise"])
		n_runs = 25_000
		H_solver = RDR.compute_uncertain_homography
		
		_, raw_statistics, TrueLabels = run_labelling_experiments(H_solver, n_runs, coeff)
		...
		roc_figure_path = DATA_PATH * "/" * "ROC_Curves_analysis_" * noise_label * ".pdf"
...
# ╔═╡ 2f59299f-af5d-4d8c-a6fa-2b1c4a521275
