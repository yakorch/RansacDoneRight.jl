...
# ╔═╡ 240c3f95-2bf8-4211-bc9e-0c61227e34c7
using LaTeXStrings
...
begin
    n_corresps_slider = @bind n_corresps Slider(5:1:200, 25, true)
    md"""
...
begin
	using Random
	shuffle!(all_correspondences)
	
    curr_cov_matrix = [
        1 0.5;
        0.5 1]
    long_run, forw_residuals = RDR.monte_carlo_cov_residuals(all_correspondences, one_long_run_times, curr_cov_matrix, false)

    Plots.plot(100 .* long_run, seriestype=:histogram, bins=length(all_correspondences) - 4, size=(800, 600), xlabel="Relative Error, %", ylabel="Frequency", label="Monte Carlo simulations vs First-Order Error Propagation")
end

# ╔═╡ fc2c09c4-b116-42cf-acc0-611eeaff2707
# Setup for a professional look
function PlotEmpiricalAndTheoreticalPDFs3D(data::Vector{Vector{Float64}})
    # Convert data to a matrix
    data_matrix = hcat(data...)'

    # Perform Kernel Density Estimation
    kde_estimation = kde(data_matrix)

    # Set mean and compute covariance
    mu = [0, 0]
    sigma = cov(data_matrix)
    dist = MvNormal(mu, sigma)

    # Define grid for plotting
    xgrid = range(minimum(data_matrix[:,1])-3, stop=maximum(data_matrix[:,1])+3, length=100)
    ygrid = range(minimum(data_matrix[:,2])-3, stop=maximum(data_matrix[:,2])+3, length=100)

    # Evaluate KDE PDF over the grid
    pdf_kde = [pdf(kde_estimation, x, y) for y in ygrid, x in xgrid]
    z = [pdf(dist, [x, y]) for x in xgrid, y in ygrid]

    # Create 3D surface plots with a professional color map
    cmap = :viridis  # A good choice for colorblind-friendly and grayscale-printable plots

    p1 = surface(xgrid, ygrid, z, title="Normal Distribution PDF", xlabel=L"x", ylabel=L"y", zlabel=L"Density",
                 camera=(30, 30), c=cmap, legend=false)
    p2 = surface(xgrid, ygrid, reshape(pdf_kde, length(ygrid), length(xgrid)), title="Empirical KDE PDF", xlabel=L"x", ylabel=L"y", zlabel=L"Density",
                 camera=(30, 30), c=cmap, legend=false)

    # Customizing plot appearance
    plot(p1, p2, layout=(1,2), size=(1200, 600), legend=:topright)
end


# ╔═╡ 14bca551-8fc9-43e6-a268-a087af285257
# ╠═╡ disabled = true
#=╠═╡
begin
	using KernelDensity
	# PlotEmpiricalAndTheoreticalPDFs3D(forw_residuals)
end
  ╠═╡ =#
...
# ╔═╡ 9a457dbb-7067-4c5c-9af6-93c2c06fe229
md"""
### Used minimal set:
"""
