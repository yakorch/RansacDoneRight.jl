export compute_residual, compute_uncertain_residuals


"""
Saves the residual in `r_container` ∈ R⁴.
"""
function _compute_residual!(r_container::MVector{4,T}, H, lu_H::Union{LinearAlgebra.LU,StaticArrays.LU}, u::SVector{2,T}, v::SVector{2,T}) where {T<:Real}
...
"""
function _compute_residual(H, lu_H::Union{LinearAlgebra.LU,StaticArrays.LU}, u::SVector{2,T}, v::SVector{2,T}) where {T<:Real}
    r = zeros(MVector{4,Float64})
    _compute_residual!(r, H, lu_H, u, v)
    return r
end


function compute_residual(H, lu_H::StaticArrays.LU, correspondence::Correspondence{Float64})
    u::SVector{2,Float64} = @view correspondence.p₁.point_coords[1:2]
    v::SVector{2,Float64} = @view correspondence.p₂.point_coords[1:2]
    return _compute_residual(H, lu_H, u, v)
end
...
"""
function _compute_residual_autodiff_wrapper!(r_container::MVector{4,T}, h_and_points::MVector{13,T}) where {T<:Real}
    h::SVector{9,T} = @view h_and_points[1:9]
    u::SVector{2,T}, v::SVector{2,T} = (@view h_and_points[10:11]), (@view h_and_points[12:end])

    H = reshape(h, (3, 3))

    _compute_residual!(r_container, H, lu(H), u, v)
    return
end
...
"""
function compute_uncertain_residuals(uncertain_H::UncertainHomography{Float64}, correspondences::V)::Vector{UncertainResidual{Float64}} where {V<:AbstractVector{Correspondence{Float64}}}
    @assert length(correspondences) > 0 "No correspondences to compute the residuals for :("

    uncertain_residuals = Vector{UncertainResidual{Float64}}(undef, length(correspondences))
    ...
    for (i, corresp) in enumerate(correspondences)
        ...
        Jᵣ = ForwardDiff.jacobian(_compute_residual_autodiff_wrapper!, residual, arguments_vector)
        ...
        uncertain_residuals[i] = UncertainResidual(residual, Σᵣ)
    end
    uncertain_residuals
end
